...
export default function SignIn() {
  ...
  const signInWithGoogle = () => {
    ...
    signInWithPopup(auth, provider)
      .then(() => {
        const project_id = "KkqDL3DyK4wSmCKPIREz";
  ...
  return (
    <>
      <div className="relative h-[115vh]">
        <img
          src="src/assets/hero_background.jpeg"
          className="w-full h-full filter grayscale object-cover object-center"
        />
        <div className="absolute inset-0 bg-gradient-to-r from-indigo-500 to-teal-300 opacity-90"></div>
        <div className="absolute inset-0 flex flex-col justify-center items-center p-10">
          ...
          <div className="flex flex-col sm:flex-row sm:space-x-10 space-y-4 sm:space-y-0 mr-auto sm:mr-0">
            ...
            <a
              href="#about"
              className="w-52 h-12 border-2 font-semibold text-white shadow rounded-2xl hover:scale-105 flex items-center justify-center"
            >
              Learn More
            </a>
          </div>
      ...
      <div id="about" className="top-0 left-0 w-full h-0 -mt-24">
        <div className="flex flex-col items-center space-y-10 lg:space-y-0 lg:flex-row place-content-evenly">
          <div className="flex flex-col bg-white h-56 w-48 rounded-full flex items-center justify-center z-50 font-bold">
            <img src="src/assets/collaborate_icon.png" className="w-32" />
            Collaborate
          </div>
          <div className="flex flex-col bg-white h-56 w-48 rounded-full flex items-center justify-center z-50 font-bold">
            <img src="src/assets/managetasks_icon.png" className="w-32" />
            Manage Tasks
          </div>
          <div className="flex flex-col bg-white h-56 w-48 rounded-full flex items-center justify-center z-50 font-bold">
            <img src="src/assets/meetgoals_icon.png" className="w-24 m-4" />
            Meet Goals
          </div>
        </div>
      </div>

      <div className="h-screen bg-orange-50 m-8 mt-32 text-3xl pt-48 p-8">
        Focus on what matters to your team
      </div>

      <div className="flex flex-col items-center text-3xl text-center m-10">
        Elevate your workflow, your way.
        <a
          href="#top"
          className="w-56 h-10 bg-gradient-to-r from-indigo-500 to-teal-500 font-semibold text-white px-2 shadow text-xl rounded-2xl hover:scale-105 mt-8 flex items-center justify-center shadow-xl"
        >
          Get started for FREE
        </a>
      </div>

      <div className="h-16 bg-stone-700 text-white text-sm flex items-center justify-end space-x-8 px-4">
        ...
        <span>Copyright &#169; 2023 Project Manager. All rights reserved.</span>
      </div>
    </>
  );
}
